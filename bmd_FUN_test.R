...
Y <- mean + rnorm(length(mean),0,0.01)
#Q <- single_continuous_fit(as.matrix(D),as.matrix(Y),sstat = F,BMR = 1.0 ,model_type="FUNL",distribution = "normal",fit_type = "laplace")

system.time({fit<-ma_continuous_fit(D*1000,Y,fit_type="mcmc",samples=25000,burnin=2500,BMR=2.0)})
system.time({fit2<-ma_continuous_fit(D*1000,Y,fit_type="laplace",BMR=2.0)})
