...
class cBMDModel : public statModel<LL, PR> {
 public:
  cBMDModel(LL t_L, PR t_PR, std::vector<bool> b_fixed, std::vector<double> d_fixed, bool I)
      : statModel<LL, PR>(t_L, t_PR, b_fixed, d_fixed), isInc(I) {};

  bool isIncreasing() { return isInc; }

  int type_of_profile(contbmd TYPE) { return this->log_likelihood.type_of_profile(TYPE); };

  int parameter_to_remove(contbmd TYPE) { return this->log_likelihood.parameter_to_remove(TYPE); };
  ...
  double equality_constraint(
      Eigen::MatrixXd theta, double *grad, contbmd BMDType, double BMD, double BMRF, double advP
  );
  ...
  //
  ////////////////////////////////////////////////////////////////////
  Eigen::MatrixXd starting_value(
      Eigen::MatrixXd theta, contbmd BMDType, double BMD, double BMRF, bool isInc, double tail_prob,
      std::vector<double> lb, std::vector<double> ub
  ) {
    return this->log_likelihood.starting_value(theta, BMDType, BMD, BMRF, isInc, tail_prob, lb, ub);
  };

  std::vector<double> bound_fix(
      std::vector<double> x, contbmd BMDType, double BMRF, double tail_prob, double BMD, bool isInc
  );
...
////////////////////
template <class LL, class PR>
std::vector<double> cBMDModel<LL, PR>::bound_fix(
    std::vector<double> x, contbmd BMDType, double BMRF, double tail_prob, double BMD, bool isInc
) {
  switch (BMDType) {
...
/*
 *Function: double equality_constraint(Eigen::MatrixXd theta, double *grad, double BMD, double BMRF,
 *double advP); Purpose : Give the current value of the inequality constraint Output: Double that
 *describes the current value of the inequality
 */
template <class LL, class PR>
double cBMDModel<LL, PR>::equality_constraint(
    Eigen::MatrixXd theta, double *grad, contbmd BMDType, double BMD, double BMRF, double advP
) {
  ...
  if (grad != NULL) {
    Eigen::MatrixXd GRAD =
        this->log_likelihood.eqConst_gradient(theta, BMDType, BMD, BMRF, true, advP);
 ...
 */
template <class LL, class PR>
double cBMDModel<LL, PR>::returnBMD(
    Eigen::MatrixXd theta, contbmd BMDType, double BMRF, double advP
) {
  // set all the fixed parameters to their value
  for (int i = 0; i < this->isFixed.size(); i++) {
    if (this->isFixed[i]) {
...
template <class LL, class PR>
optimizationResult cfindMAX_W_EQUALITY(
    cBMDModel<LL, PR> *M, Eigen::MatrixXd start, contbmd BMDType, double BMD, double BMRF,
...
double neg_pen_likelihood_contbound(unsigned n, const double *b, double *grad, void *data) {
  ...
  x = model->sm->bound_fix(
      x, model->BMDType, model->BMR, model->add_info, model->cBMD, model->isInc
  );
  // cout << endl;
...
// findMaxW_EQUALITY <- for continuous bmd models
template <class LL, class PR>
optimizationResult cfindMAX_W_BOUND(
    cBMDModel<LL, PR> *M, Eigen::MatrixXd start, contbmd BMDType, double BMD, double BMRF,
    bool isInc, double tail_prob
) {
  optimizationResult oR;
  ...
  while (!good_opt && opt_iter <= 2) {
    ...
    DEBUG_LOG(
        file, "\topt_iter= " << opt_iter << ", result= " << result << ", minf= " << minf
                             << ", good_opt= " << good_opt
    );
  }
...
////////////////////////////////////////////////////////////////////////////////
template <class LL, class PR>
Eigen::MatrixXd profile_cBMDNC(
    cBMDModel<LL, PR> *M, contbmd BMDType, const double BMD, const double BMRF,
    const double tail_prob, const double BMDchange, const double totalChange, bool isIncreasing
) {
  ...
  while (fabs(MAP_LIKE - CLIKE) < totalChange && max_iters < 300) {
    ...
    if (M->type_of_profile(BMDType) == PROFILE_EQUALITY) {
      oR = cfindMAX_W_EQUALITY<LL, PR>(M, parms, BMDType, CBMD, BMRF, isIncreasing, tail_prob);
    } else {
      oR = cfindMAX_W_BOUND<LL, PR>(M, parms, BMDType, CBMD, BMRF, isIncreasing, tail_prob);
    }
