...
class dich_logisticModelNC : public binomialBMD {
 public:
  ...
  Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd X) {
    ...
    for (int i = 0; i < X.rows(); i++) p(i, 0) = LOGISTIC_MEAN(g, b, X(i, 1));
  ...
  std::vector<double> fixConstraintLB(
    ...
    if (isExtra) {
      ...
      // temp = (lb*log(BMD) - Z) *1.2; // add a little extra add a little extra so the constraint
      // is satisfied
      x[0] = theta(0, 0);
    } else {
  ...
  std::vector<double> fixConstraintUB(
    ...
    if (isExtra) {
      Z = LOGISTIC_EXTRA_Z(a, BMR);
      // temp = (Z - ub * log(BMD))*1.01; // add a little extra add a little extra so the constraint
      // is satisfied
      x[0] = theta(0, 0);
      // x[1] = theta(1, 0) - temp;
    } else {
  ...
  Eigen::MatrixXd XgivenD(double d) {
    Eigen::MatrixXd rV(1, 2);
    rV << 1.0, d;  // by default the
                   // dose is the last parameter in X
    return rV;
  }

  double compute_BMD_EXTRA_NC_INEQUALITY(
  ...
  double compute_BMD_ADDED_NC_INEQUALITY(
      Eigen::MatrixXd theta, double BMD, double BMR, double inequality, bool geq, double* grad
  ) {
    logistic_inequality M;
    M.inequality = inequality;
