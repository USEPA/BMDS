...
class dBMDModel : public statModel<LL, PR> {
 public:
  dBMDModel(LL t_L, PR t_PR, std::vector<bool> b_fixed, std::vector<double> d_fixed)
      : statModel<LL, PR>(t_L, t_PR, b_fixed, d_fixed) {};

  std::vector<double> fixConstraintLB(
      Eigen::MatrixXd theta, double BMD, double BMR, bool isExtra, double lb
  ) {
  ...
  std::vector<double> fixConstraintUB(
      Eigen::MatrixXd theta, double BMD, double BMR, bool isExtra, double ub
  ) {
    // set all the fixed parameters to their value
    for (int i = 0; i < this->isFixed.size(); i++) {
      if (this->isFixed[i]) {
  ...
  bool fixedNC_BMDformula() { return statModel<LL, PR>::log_likelihood.fixedNC_BMDformula(); }

  int removedParameter() { return statModel<LL, PR>::log_likelihood.parameterRemoved(); }
  ...
  virtual double inequality_extra(
      Eigen::MatrixXd theta, double BMD, double BMR, double inequality, bool geq, double *grad
  ) {
    // set all the fixed parameters to their value
    for (int i = 0; i < this->isFixed.size(); i++) {
      if (this->isFixed[i]) {
    ...
    return statModel<LL, PR>::log_likelihood.compute_BMD_EXTRA_NC_INEQUALITY(
        theta, BMD, BMR, inequality, geq, grad
    );
  }

  virtual double inequality_added(
      Eigen::MatrixXd theta, double BMD, double BMR, double inequality, bool geq, double *grad
  ) {
    // set all the fixed parameters to their value
    for (int i = 0; i < this->isFixed.size(); i++) {
    ...
    return statModel<LL, PR>::log_likelihood.compute_BMD_ADDED_NC_INEQUALITY(
        theta, BMD, BMR, inequality, geq, grad
    );
  }
};
...
template <class LL, class PR>
double inequality_constraint(unsigned n, const double *b, double *grad, void *data) {
  inequalityInfo<LL, PR> *model = (inequalityInfo<LL, PR> *)data;
  Eigen::MatrixXd theta(n, 1);
  for (int i = 0; i < n; i++) theta(i, 0) = b[i];

  if (model->isExtra) {
    return model->sm->inequality_extra(
        theta, model->cBMD, model->BMR, model->inequality, model->geq, grad
    );
  } else {
    return model->sm->inequality_added(
        theta, model->cBMD, model->BMR, model->inequality, model->geq, grad
    );
  }
}
...
template <class LL, class PR>
double inequality_constraint_general(unsigned n, const double *b, double *grad, void *data) {
  inequalityInfo<LL, PR> *model = (inequalityInfo<LL, PR> *)data;
  Eigen::MatrixXd theta(n, 1);
  for (int i = 0; i < n; i++) theta(i, 0) = b[i];  // simple addition
...
template <class LL, class PR>
optimizationResult findMAX_W_EQUALITY(
    dBMDModel<LL, PR> *M, Eigen::MatrixXd start,
  ...
  bool lbsatisfied = true;
  bool ubsatisfied = true;
...
// Output: statMod<LL,PR> *M - The model with it's MAP parameter set.
template <class LL, class PR>
optimizationResult findMAX(
    dBMDModel<LL, PR> *M, Eigen::MatrixXd start,
    const bool isExtra,  // true if it is false if it is added
    double BMD, double BMR, const int iter, nlopt::algorithm algorithm
) {
  optimizationResult oR;
...
////////////////////////////////////////////////////////////////////////////////////
template <class LL, class PR>
std::list<Eigen::MatrixXd> fit_profileLogic(
    dBMDModel<LL, PR> *M, Eigen::MatrixXd start,
    const bool isExtra,  // true if it is false if it is added
    double BMD, double BMR, const int iter, nlopt::algorithm algorithm
) {
  ...
  try {
    ...
    oR = findMAX<LL, PR>(M, start, isExtra, BMD, BMR, iter, algorithm);
    ...
    result(0, 0) = DBL_MAX;
    result(1, 0) = BMD;
    result(2, 0) = NLOPT_ROUNDOFF_LIMITED;
    bad_result = true;
...
//	       double BMDchange      - %Change in the BMD to increment each time
//		   double totalChange    - totalChange in penalized likelihood before one stops
//          bool   robust         - true if we do a robust search of the optimization space, false
//          otherwise
////////////////////////////////////////////////////////////////////////////////
template <class LL, class PR>
Eigen::MatrixXd profile_BMDNC(
    dBMDModel<LL, PR> *M,
    const bool isExtra,  // true if it is false if it is added
    const double BMD, const double BMR, const double BMDchange, const double totalChange,
    bool robust
)

{
  ...
  while (fabs(MAP_LIKE - CLIKE) < totalChange && max_iters < 500 && CBMD > 1e-8) {
    ...
    temp1 = fit_profileLogic<LL, PR>(M, parms, isExtra, CBMD, BMR, 10000, nlopt::LN_COBYLA);

    // if the profile fit is called using the
    // robust option then we call the second optimizer
    // second opinion
    if (true) {
      temp2 = fit_profileLogic<LL, PR>(M, parms, isExtra, CBMD, BMR, 250, nlopt::LD_MMA);
      ret1 = temp1.front();
      ret2 = temp2.front();
  ...
  while (max_iters2 < 200 && CBMD < 2.5 * max_dose && fabs(MAP_LIKE - CLIKE) < totalChange) {
    // fit the first profile likelihood option
    temp1 = fit_profileLogic<LL, PR>(M, parms, isExtra, CBMD, BMR, 10000, nlopt::LN_COBYLA);
    ...
    // second opinion
    if (robust) {
      temp2 = fit_profileLogic<LL, PR>(M, parms, isExtra, CBMD, BMR, 150, nlopt::LD_MMA);
  ...
  returnMat.col(0).array() = round(round(1e4 * returnMat.col(0).array()) - round(1e4 * MAP_LIKE)) /
