...
def generate_extra_plots(session: "pybmds.Session"):
    ...
    for model in rows:
        ...
        for param, value in zip(model["parameters"], model["parameter_vals"], strict=False):
            ...
            def calc_first_value(row):
                ...
                if is_ilc_plus and not phi_is_zero:
                    ...
                    return (
                        ...
                        else math.factorial(int(row["litter_size"]))
                        / (math.factorial(i) * math.factorial(int(row["litter_size"]) - i))
                    )
                ...
                if is_ilc_plus and not phi_is_zero:
                    ...
                    return (
                        ...
                        * scipy.special.gamma(row["litter_size"] - i + row["beta"])
                        / (
                            scipy.special.gamma(i + 1)
                            * scipy.special.gamma(row["litter_size"] - i + 1)
                        )
                    )
                else:
                    return (row["est_prob"] ** i) * (
                        (1 - row["est_prob"]) ** (row["litter_size"] - i)
                    )
    ...
    plot_data[["obs_proportion", "obs_ci_lower", "obs_ci_upper"]] = plot_data.apply(
        lambda row: calculate_plotting(row["obs_count"], row["total_litters_per_dose"]),
        axis=1,
        result_type="expand",
    )
