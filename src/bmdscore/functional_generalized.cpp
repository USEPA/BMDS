...
Eigen::VectorXd binomial_cmstage2(const Eigen::VectorXd& params, 
  ...
  out = out.unaryExpr([&g,&b1,&b2](double xx){return g + (1-g)*(1-exp(-b1*xx-b2*xx*xx));});
  return out;
}
...
//  double a = R::qnorm((p1-p0)/(1-p0),0,1,true,false);
  ...
  Eigen::VectorXd out = X.col(0);
   ...
   out = out.unaryExpr([&p0,&b,&a](double xx){   if (xx == 0){
                                                        return p0;
                                                  }
...
Eigen::VectorXd binomial_chill(const Eigen::VectorXd& params, 
  ...
  out = out.unaryExpr([&g,&b,&a,&v](double xx){    if (xx == 0){
                                                        return g;
                                                    }
...
Eigen::VectorXd binomial_cgamma(const Eigen::VectorXd& params, 
  ...
  //out = out.unaryExpr([&p0,&b,&alpha](double xx){return p0+(1-p0)*( R::pgamma(b*xx,alpha,1.0,true,false));});
  out = out.unaryExpr([&p0,&b,&alpha](double xx){return p0+(1-p0)*( gsl_cdf_gamma_P(b*xx,alpha,1.0));});
...
double log_beta_pdf(double x,
                   ...
                   double beta){
        double returnV = 0.0; 
        if (x <= 0 || x >= 1.0) {return  -std::numeric_limits<double>::infinity();}
...
Eigen::VectorXd binomial_mstage3(const Eigen::VectorXd& params, 
  ...
  out = out.unaryExpr([&g,&b1,&b2,&b3](double xx){return g + (1-g)*(1-exp(-b1*xx-b2*xx*xx-b3*xx*xx*xx));});
  return out;
}
...
Eigen::VectorXd hill_cpp2(const Eigen::VectorXd& params, const Eigen::MatrixXd& X){
  ...
  double d = params[3];
  Eigen::VectorXd out = X.col(0);
  out = out.unaryExpr([&a,&b,&c,&d](double xx){return a*(1.0 + (c - 1.0)*(1.0 - pow(b, d) / (pow(b,d) + pow(xx, d))));});
  return out;
}
...
Eigen::VectorXd exponential_cpp2(const Eigen::VectorXd& params, const Eigen::MatrixXd& X){
  ...
  double d = params[3];
  Eigen::VectorXd out = X.col(0);
  out = out.unaryExpr([&a,&b,&c,&d](double xx){return a*(1.0 + (c - 1.0)*(1.0 - exp(-b * pow(xx, d))));});
  return out;
}
...
Eigen::VectorXd continuous_power_transform(const Eigen::VectorXd& params, const Eigen::MatrixXd& X) {
  ...
  Eigen::VectorXd out = X.col(0);
  out = out.unaryExpr([&g, &v, &n](double xx) {
    return g + v * std::pow(xx, n);
  });
...
Eigen::VectorXd logistic_cpp2(const Eigen::VectorXd& params, const Eigen::MatrixXd& X){
  ...
  double d = params[3];
  Eigen::VectorXd out = X.col(0);
  out = out.unaryExpr([&a,&b,&c,&d](double xx){return c / (1.0 + exp(-1.0*a - b*pow(xx,d)));});
  return out;
}
...
Eigen::MatrixXd transformed_slice_sampler_cpp3(const Eigen::MatrixXd& Y,
  ...
  // std::function<double(Eigen::VectorXd &, const Eigen::MatrixXd& ,const Eigen::VectorXd &)> target= &posterior_d_cpp;
                          ...
                          const Eigen::MatrixXd& prior_spec)
{
  ...
  for (int i = 0; i < theta.size(); i++)
  {
    ...
    case 1:
      ...
      sd = prior_spec(i, 2);
      returnV += -log(sd) - 0.5 * mean * mean / (sd * sd) + double(theta.size()) * log(0.5 * M_2_SQRTPI * M_SQRT1_2);
      break;
    case 2:
      ...
      if (theta[i] <= 0 || theta[i] > 20){
          ...
          sd = prior_spec(i, 2);
          returnV += -log(sd) - log(theta[i]) - 0.5 * mean * mean / (sd * sd) + double(theta.size()) * log(0.5 * M_2_SQRTPI * M_SQRT1_2);
      }
      break;
    case 3: 
      returnV += log_beta_pdf(theta[i],
                              prior_spec(i, 1),
                              prior_spec(i, 2)) ;
      break; 
    case 4: // gamma distribution for dirichlet
      returnV += log_gamma_pdf(theta[i],
                               prior_spec(i, 1),
                               prior_spec(i, 2)) ;
      break;
...
double ll_binomial(Eigen::VectorXd params, 
    ...
    ll += (Y.col(0).array()*mean.array().log() + (Y.col(1).array()-Y.col(0).array())*(1-mean.array()).log()).array().sum(); 
    if (std::isnan(ll)){ 
                             ...
                             const ptr2& nly)
{
  ...
  for (int i = 0; i < Y.rows(); ++i) {
    ...
    ll += -0.5 * n * std::log(2 * M_PI) - 0.5 * n * std::log(sigma2)
...
Eigen::MatrixXd run_latentslice_functional_general(const Eigen::MatrixXd &X, 
  ...
  //get the nonlinearity type (1 = Hill, 2 = Power, 3 = Exponential, 5 = Logistic, otherwise use Linear)
  ...
  std::function<double(Eigen::VectorXd, 
                       ...
                       const Eigen::MatrixXd&, 
                       std::function<double(Eigen::VectorXd, const Eigen::MatrixXd&, const Eigen::MatrixXd&, const ptr2&)>,
                       std::function<double(Eigen::VectorXd, const Eigen::MatrixXd&)>, const ptr2&)> postt = full_postt;
  ...
  std::function<double(Eigen::VectorXd, const Eigen::MatrixXd&, const Eigen::MatrixXd&, const ptr2&)> logli;
  if(ll_typ == 55){
    logli = ll_continuous;
  }else if (ll_typ == 56){
  ...
  // initial run
  Eigen::MatrixXd init_samps = initial_slice_sampler_cpp3(Y, initial_val, postt, priorr, cov, X, burnin_samples, LAM, logli, priii, nonlinn);
...
//                                               cov_eta, function_return, X, burnin_samples, LAM, logli, priii, nonlinn);
    init_samps = transformed_slice_sampler_cpp3(Y, new_start, postt, priorr,
                                               CM, cov, betas, knots, X, burnin_samples, LAM, logli, priii, nonlinn);
  }
//  List function_return = compute_transform_f_lag1_cpp3(init_samps, qtiles);
  //betas.setZero();
  //knots.setZero();
...
//  List cov_eta = compute_cov_eta_cpp3(init_samps, function_return);
  ...
  Eigen::VectorXd new_start = init_samps.row(burnin_samples - 1);
//  init_samps = transformed_slice_sampler_cpp3(Y, new_start, postt, priorr,
//                                             cov_eta, function_return, X, keep_samples, LAM, logli, priii, nonlinn);
  init_samps = transformed_slice_sampler_cpp3(Y, new_start, postt, priorr,
                                             CM, cov, betas, knots, X, keep_samples, LAM, logli, priii, nonlinn);
  return init_samps;
}
