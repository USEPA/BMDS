...
template <class LL>
Eigen::MatrixXd X_gradient(
    Eigen::MatrixXd theta, Eigen::MatrixXd Y, Eigen::MatrixXd D, int degree = 1
) {
...
template <class LL>
Eigen::MatrixXd X_compute_mean(
    Eigen::MatrixXd Y, Eigen::MatrixXd D, Eigen::MatrixXd parms, int degree = 1
) {
  LL data_likelihood(Y, D, degree);
...
template <class LL>
Eigen::MatrixXd X_cov(Eigen::MatrixXd theta, Eigen::MatrixXd Y, Eigen::MatrixXd D) {
...
void estimate_ma_MCMC(
    dichotomousMA_analysis *MA, dichotomous_analysis *DA, dichotomousMA_result *res, ma_MCMCfits *ma
);
...
void estimate_ma_laplace(
    dichotomousMA_analysis *MA, dichotomous_analysis *DA, dichotomousMA_result *res
);

/* Function: estimate_sm_laplace
 * Purpose:  This function performs a single model estimate for dichotomous data  This is done using
 * Laplace estimation. Input  :
 *
 *  @dichotomous_analysis   - Pointer to memory containing information about the basic
 ...
 *                  that use the exact same estimation algorithms for continuous data.
 */
void estimate_sm_laplace(
    dichotomous_analysis *DA, dichotomous_model_result *res, bool do_a_rescale = true
);
...
void estimate_sm_mcmc(
    dichotomous_analysis *DA, dichotomous_model_result *res, bmd_analysis_MCMC *mcmc,
...
extern "C" {
#  endif
void estimate_sm_laplace_dicho(
    struct dichotomous_analysis *DA, struct dichotomous_model_result *res, bool do_a_rescale
);

void estimate_ma_laplace_dicho(
    struct dichotomousMA_analysis *MA, struct dichotomous_analysis *DA,
    struct dichotomousMA_result *res
);

void compute_dichotomous_pearson_GOF(
    struct dichotomous_PGOF_data *data, struct dichotomous_PGOF_result *res
);
