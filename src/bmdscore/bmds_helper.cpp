...
void BMDS_ENTRY_API __stdcall runBMDSContAnalysis(
    ...
    bool *detectAdvDir, bool *restricted, bool *countAllParmsOnBoundary
) {
  bmdsRes->BMD = BMDS_MISSING;
  bmdsRes->BMDL = BMDS_MISSING;
  bmdsRes->BMDU = BMDS_MISSING;
...
void BMDS_ENTRY_API __stdcall pythonBMDSDicho(
  ...
  runBMDSDichoAnalysis(
      &anal, &res, &pyRes->gof, &pyRes->bmdsRes, &pyRes->aod, &pyAnal->countAllParmsOnBoundary
  );
...
void BMDS_ENTRY_API __stdcall pythonBMDSCont(
  ...
  runBMDSContAnalysis(
      ...
      &pyAnal->restricted, &pyAnal->countAllParmsOnBoundary
  );
...
void BMDS_ENTRY_API __stdcall pythonBMDSNested(
  ...
  int df = pyRes->nparms;
  if (!pyAnal->countAllParmsOnBoundary) {
    df -= numBounded;
  }
  double pv;
  if (dev < 0.0) {
...
// TODO: clean up comments after debugging is complete
{
  ...
  for (;;) /* Main iteration loop	*/
  {
    ...
    if (fabs(new_step) < tol_act) /* Adjust the step to be not less*/ { if
	    (new_step > (double)0) /* than tolerance		*/ new_step =
		    tol_act; else new_step = -tol_act; }

    a = b; fa = fb; /* Save the previous approx.	*/ b += new_step; fb =
	    (*f)(Parms, b, ck, objData);               /* Do step to a new
							  approxim.	*/ if
    ((fb > 0 && fc > 0) || (fb < 0 && fc < 0)) { /* Adjust c for it to have a
						    sign*/ c = a; fc = fa; /*
									      opposite
									      to
									      that
									      ...
									      b
									      */
    } pass++; if (pass > maxPass) return BMDS_MISSING; } }

template <typename T> void printElement(std::stringstream &ss, T t, const int
		&width) { const char separator = ' '; ss << left << setw(width)
	<< setfill(separator) << t; }

std::string printBmdsStruct(struct testsOfInterest *TOI, bool print) { const
	int nameWidth = 20; const int numWidth = 9; std::stringstream ss;

  std::vector<std::string> desc = { "Test1: A2 vs R", "Test2: A1 vs R", "Test3: A3 vs A2", "Test4: Fitted vs A3" };

  ss << std::endl << "Struct: testsOfInterest" << std::endl;

  printElement(ss, "Desc", nameWidth); printElement(ss, "llRatio", numWidth);
  printElement(ss, "DF", numWidth); printElement(ss, "pVal", numWidth); ss <<
	  std::endl; for (int i = 0; i < desc.size(); i++) { printElement(ss,
			  desc[i], nameWidth); printElement(ss,
				  TOI->llRatio[i], numWidth); printElement(ss,
					  TOI->DF[i], numWidth);
	  printElement(ss, TOI->pVal[i], numWidth); ss << std::endl; }

  if (print) std::cout << ss.str() << std::endl;

  return ss.str(); }

std::string printBmdsStruct(struct continuous_AOD *AOD, bool print) { const int
	nameWidth = 13; const int numWidth = 10; std::stringstream ss;

  std::vector<std::string> desc = {"A1 Model", "A2 Model", "A3 Model", "Fitted Model", "R Model"};

  ss << std::endl << "Struct: continuous_AOD" << std::endl; printElement(ss,
		  "Model", nameWidth); printElement(ss, "LL", numWidth);
  printElement(ss, "nParms", numWidth); printElement(ss, "AIC", numWidth); ss
	  << std::endl; for (int i = 0; i < desc.size(); i++) {
		  printElement(ss, desc[i], nameWidth); printElement(ss,
				  AOD->LL[i], numWidth); printElement(ss,
					  AOD->nParms[i], numWidth);
		  printElement(ss, AOD->AIC[i], numWidth); ss << std::endl; }
...
std::string printBmdsStruct(struct dicho_AOD *AOD, bool print) { const int
	nameWidth = 13; const int numWidth = 10; std::stringstream ss;

  std::vector<std::string> desc = {"Full Model", "Fitted Model", "Reduced Model"};

  ss << std::endl << "Struct: dicho_AOD" << std::endl; printElement(ss,
		  "Model", nameWidth); printElement(ss, "LL", numWidth);
  printElement(ss, "nParms", numWidth); printElement(ss, "Deviance", numWidth);
  printElement(ss, "DF", numWidth); printElement(ss, "P Value", numWidth); ss
	  << std::endl;

  printElement(ss, "Full Model", nameWidth); printElement(ss, AOD->fullLL,
		  numWidth); printElement(ss, AOD->nFull, numWidth);
  printElement(ss, "-", numWidth); printElement(ss, "-", numWidth);
  printElement(ss, "NA", numWidth); ss << std::endl; printElement(ss, "Fitted Model", nameWidth); printElement(ss, AOD->fittedLL,
		  numWidth); printElement(ss, AOD->nFit, numWidth);
  printElement(ss, AOD->devFit, numWidth); printElement(ss, AOD->dfFit,
		  numWidth); printElement(ss, AOD->pvFit, numWidth); ss <<
	  std::endl; printElement(ss, "Reduced Model", nameWidth);
  printElement(ss, AOD->redLL, numWidth); printElement(ss, AOD->nRed,
		  numWidth); printElement(ss, AOD->devRed, numWidth);
  printElement(ss, AOD->dfRed, numWidth); printElement(ss, AOD->pvRed,
		  numWidth); ss << std::endl;

  if (print) std::cout << ss.str() << std::endl;

  return ss.str(); }

std::string printBmdsStruct(struct BMDS_results *bmdsRes, bool print) { const
	int colWidth = 11;

  std::stringstream ss;

  ss << std::endl << "Struct: BMDS_results" << std::endl; ss << "BMD:" <<
	  bmdsRes->BMD << std::endl; ss << "BMDL:" << bmdsRes->BMDL <<
	  std::endl; ss << "BMDU:" << bmdsRes->BMDU << std::endl; ss << "AIC:"
	  << bmdsRes->AIC << std::endl; ss << "BIC_equiv:" <<
	  bmdsRes->BIC_equiv << std::endl; ss << "chisq:" << bmdsRes->chisq <<
	  std::endl;

  printElement(ss, "bounded", colWidth); printElement(ss, "stdErr", colWidth);
  if (bmdsRes->lowerConf.size() > 0) { printElement(ss, "lowerConf", colWidth);
	  printElement(ss, "upperConf", colWidth); } ss << std::endl;

  for (int i = 0; i < bmdsRes->bounded.size(); i++) { printElement(ss,
		  (bmdsRes->bounded[i] ? "true" : "false"), colWidth);
  printElement(ss, bmdsRes->stdErr[i], colWidth); if (bmdsRes->lowerConf.size()
		  > 0) { printElement(ss, bmdsRes->lowerConf[i], colWidth);
	  printElement(ss, bmdsRes->upperConf[i], colWidth); } ss << std::endl;
  }

  ss << "validResult:" << (bmdsRes->validResult ? "true" : "false") <<
	  std::endl; ss << "slopeFactor:" << bmdsRes->slopeFactor << std::endl;

  if (print) std::cout << ss.str() << std::endl;

  return ss.str(); }

std::string printBmdsStruct(struct BMDSMA_results *bmdsRes, bool print) { const
	int colWidth = 10; std::stringstream ss;

  ss << std::endl << "Struct: BMDSMA_results" << std::endl; ss << "BMD_MA:" <<
	  bmdsRes->BMD_MA << std::endl; ss << "BMDL_MA:" << bmdsRes->BMDL_MA <<
	  std::endl; ss << "BMDU_MA:" << bmdsRes->BMDU_MA << std::endl;

  printElement(ss, "model", colWidth); printElement(ss, "BMD", colWidth);
  printElement(ss, "BMDL", colWidth); printElement(ss, "BMDU", colWidth); ss <<
	  std::endl; for (int i = 0; i < bmdsRes->BMD.size(); i++) {
		  printElement(ss, i, colWidth); printElement(ss,
				  bmdsRes->BMD[i], colWidth); printElement(ss,
					  bmdsRes->BMDL[i], colWidth);
		  printElement(ss, bmdsRes->BMDU[i], colWidth); ss <<
			  std::endl; }

  ss << "Error bars" << std::endl; printElement(ss, "dose grp", colWidth);
  printElement(ss, "ebLower", colWidth); printElement(ss, "ebUpper", colWidth);
  ss << std::endl; for (int i = 0; i < bmdsRes->ebLower.size(); i++) {
	  printElement(ss, i, colWidth); printElement(ss, bmdsRes->ebLower[i],
			  colWidth); printElement(ss, bmdsRes->ebUpper[i],
				  colWidth); ss << std::endl; }
...
std::string printBmdsStruct(struct nestedLitterData *litter, bool print) {
	const int colWidth = 12; std::stringstream ss;

  ss << std::endl << "Struct: nestedLitterData" << std::endl;

  printElement(ss, "dose", colWidth); printElement(ss, "LSC", colWidth);
  printElement(ss, "estProb", colWidth); printElement(ss, "litterSize",
		  colWidth); printElement(ss, "expected", colWidth);
  printElement(ss, "observed", colWidth); printElement(ss, "SR", colWidth); ss
	  << std::endl; for (int i = 0; i < litter->dose.size(); i++) {
		  printElement(ss, litter->dose[i], colWidth); printElement(ss,
				  litter->LSC[i], colWidth); printElement(ss,
					  litter->estProb[i], colWidth);
		  ...
		  printElement(ss, litter->SR[i], colWidth); ss << std::endl; }
...
std::string printBmdsStruct(struct nestedBootstrap *boot, bool print) { const
	int colWidth = 10; std::stringstream ss;

  ss << std::endl << "Struct: nestedBootstrap" << std::endl;

  printElement(ss, "pVal", colWidth); printElement(ss, "perc50", colWidth);
  printElement(ss, "perc90", colWidth); printElement(ss, "perc95", colWidth);
  printElement(ss, "perc99", colWidth); ss << std::endl; for (int i = 0; i <
		  boot->pVal.size(); i++) { printElement(ss, boot->pVal[i],
		  colWidth); printElement(ss, boot->perc50[i], colWidth);
		  printElement(ss, boot->perc90[i], colWidth); printElement(ss,
				  boot->perc95[i], colWidth); printElement(ss,
					  boot->perc99[i], colWidth); ss <<
					  std::endl; }
...
std::string printBmdsStruct(struct nestedReducedData *red, bool print) { const
	int colWidth = 12; std::stringstream ss;

  ss << std::endl << "Struct: nestedReducedData" << std::endl;

  printElement(ss, "dose", colWidth); printElement(ss, "propAffect", colWidth);
  printElement(ss, "lowerConf", colWidth); printElement(ss, "upperConf",
		  colWidth); ss << std::endl; for (int i = 0; i <
			  red->dose.size(); i++) { printElement(ss,
			  red->dose[i], colWidth); printElement(ss,
				  red->propAffect[i], colWidth);
			  printElement(ss, red->lowerConf[i], colWidth);
			  printElement(ss, red->upperConf[i], colWidth); ss <<
				  std::endl; }
...
std::string printBmdsStruct(struct nestedSRData *sr, bool print) {
  ...
  ss << std::endl << "Struct: nestedSRData" << std::endl; ss << "minSR:" <<
	  sr->minSR << std::endl; ss << "avgSR:" << sr->avgSR << std::endl; ss
	  << "maxSR:" << sr->maxSR << std::endl; ss << "minAbsSR:" <<
	  sr->minAbsSR << std::endl; ss << "avgAbsSR:" << sr->avgAbsSR <<
	  std::endl; ss << "maxAbsSR:" << sr->maxAbsSR << std::endl;

  if (print) std::cout << ss.str() << std::endl;

  return ss.str(); }

std::string printBmdsStruct(struct continuous_GOF *gof, bool print) { const int
	colWidth = 12; std::stringstream ss;

  ss << std::endl << "Struct: continuous_GOF" << std::endl; ss << "n:" <<
	  gof->n << std::endl;

  printElement(ss, "dose", colWidth); printElement(ss, "size", colWidth);
  printElement(ss, "estMean", colWidth); printElement(ss, "calcMean",
		  colWidth); printElement(ss, "obsMean", colWidth);
  printElement(ss, "estSD", colWidth); printElement(ss, "calcSD", colWidth);
  printElement(ss, "obsSD", colWidth); printElement(ss, "res", colWidth);
  printElement(ss, "ebLower", colWidth); printElement(ss, "ebUpper", colWidth);
  ss << std::endl;

  for (int i = 0; i < gof->dose.size(); i++) { printElement(ss, gof->dose[i],
		  colWidth); printElement(ss, gof->size[i], colWidth);
  printElement(ss, gof->estMean[i], colWidth); printElement(ss,
		  gof->calcMean[i], colWidth); printElement(ss,
			  gof->obsMean[i], colWidth); printElement(ss,
				  gof->estSD[i], colWidth); printElement(ss,
					  gof->calcSD[i], colWidth);
  printElement(ss, gof->obsSD[i], colWidth); printElement(ss, gof->res[i],
		  colWidth); printElement(ss, gof->ebLower[i], colWidth);
  printElement(ss, gof->ebUpper[i], colWidth); ss << std::endl; }

  if (print) std::cout << ss.str() << std::endl; return ss.str(); }

std::string printBmdsStruct(struct dichotomous_GOF *gof, bool print) { const
	int colWidth = 12; std::stringstream ss;

  ss << std::endl << "Struct: dichotomous_GOF" << std::endl; ss << "n:" <<
	  gof->n << std::endl;

  printElement(ss, "expected", colWidth); printElement(ss, "residual",
		  colWidth); ss << std::endl;

  for (int i = 0; i < gof->n; i++) { printElement(ss, gof->expected[i],
		  colWidth); printElement(ss, gof->residual[i], colWidth); ss
	  << std::endl; }

  ss << "test_statistic:" << gof->test_statistic << std::endl; ss << "p_value:"
	  << gof->p_value << std::endl; ss << "df:" << gof->df << std::endl;

  ss << "Error bars" << std::endl; printElement(ss, "ebUpper", colWidth);
  printElement(ss, "ebLower", colWidth); ss << std::endl; for (int i = 0; i <
		  gof->ebLower.size(); i++) { printElement(ss, gof->ebLower[i],
		  colWidth); printElement(ss, gof->ebUpper[i], colWidth); ss <<
			  std::endl; }
...
std::string printBmdsStruct(struct fitInput *in, bool print) { const int
	colWidth = 10; const int largeColWidth = 14;

  std::stringstream ss; ss << std::endl << "Struct: fitInput" << std::endl;

  ss << "doses:" << in->doses << std::endl; ss << "Y:" << in->Y << std::endl;
  ss << "lmean0:" << in->lmean0 << std::endl; ss << "lmean1:" << in->lmean1 <<
	  std::endl; ss << "N_obs0:" << in->N_obs0 << std::endl; ss <<
	  "N_obs1:" << in->N_obs1 << std::endl; ss << "s0sq:" << in->s0sq <<
	  std::endl; ss << "s1sq:" << in->s1sq << std::endl; ss << "N_obs:" <<
	  in->N_obs << std::endl; ss << "ssq:" << in->ssq << std::endl; ss <<
	  "sign:" << (in->sign ? "True" : "False") << std::endl; ss << "iter:"
	  << in->iter << std::endl; ss << "bmr:" << in->bmr << std::endl; ss <<
	  "dist:" << in->dist << std::endl; ss << "datatype:" << in->datatype
	  << std::endl;

  if (print) std::cout << ss.str() << std::endl; return ss.str(); }

std::string printBmdsStruct(struct fitResult *out, bool print) { const int
	colWidth = 10; const int largeColWidth = 14;

  std::stringstream ss; ss << std::endl << "Struct: fitOutput" << std::endl;

  ss << "parms:" << out->parms << std::endl; ss << "int_factor:" <<
	  out->int_factor << std::endl; ss << "waic:" << out->waic <<
	  std::endl; ss << "BMD:" << out->BMD << std::endl;
  // ss << "R:" << out->R << std::endl;

  if (print) std::cout << ss.str() << std::endl; return ss.str(); }

std::string printBmdsStruct(struct python_continuous_analysis *pyAnal, bool
		print) { const int colWidth = 8; std::stringstream ss;

  ss << std::endl << "Struct: python_continuous_analysis" << std::endl; ss <<
	  "model:" << pyAnal->model << std::endl; ss << "n:" << pyAnal->n <<
	  std::endl; ss << "suff_stat:" << (pyAnal->suff_stat ? "true" :
			  "false") << std::endl; ss << "Data:" << std::endl;
  printElement(ss, "doses", colWidth); printElement(ss, "Y", colWidth); if
	  (pyAnal->suff_stat) { printElement(ss, "sd", colWidth);
		  printElement(ss, "n_group", colWidth); } ss << std::endl; for
			  (int i = 0; i < pyAnal->doses.size(); i++) {
				  printElement(ss, pyAnal->doses[i], colWidth);
				  printElement(ss, pyAnal->Y[i], colWidth); if
					  (pyAnal->suff_stat) {
						  printElement(ss,
								  pyAnal->sd[i],
								  colWidth);
  ...
  if (pyAnal->prior_cols > 0 && pyAnal->prior.size() > 0) { ss << "prior:" <<
	  std::endl; for (int i = 0; i < pyAnal->prior.size() /
			  pyAnal->prior_cols; i++) { for (int j = 0; j <
				  pyAnal->prior_cols; j++) { printElement(ss,
					  pyAnal->prior[i * pyAnal->prior_cols
					  + j], colWidth); } ss << std::endl; }
  } ss << "BMD_type:" << pyAnal->BMD_type << std::endl; ss << "isIncreasing:"
  << (pyAnal->isIncreasing ? "true" : "false") << std::endl; ss << "BMR:" <<
	  pyAnal->BMR << std::endl; ss << "tail_prob:" << pyAnal->tail_prob <<
	  std::endl; ss << "disttype:" << pyAnal->disttype << std::endl; ss <<
	  "alpha:" << pyAnal->alpha << std::endl; ss << "samples:" <<
	  pyAnal->samples << std::endl; ss << "degree:" << pyAnal->degree <<
	  std::endl; ss << "burnin:" << pyAnal->burnin << std::endl; ss <<
	  "parms:" << pyAnal->parms << std::endl; ss << "prior_cols:" <<
	  pyAnal->prior_cols << std::endl; ss << "transform_dose:" <<
	  pyAnal->transform_dose << std::endl; ss << "restricted:" <<
	  (pyAnal->restricted ? "true" : "false") << std::endl; ss <<
	  "detectAdvDir:" << (pyAnal->detectAdvDir ? "true" : "false") <<
	  std::endl; ss << "countAllParmsOnBoundary:" <<
	  (pyAnal->countAllParmsOnBoundary ? "true" : "false") << std::endl;

  if (print) std::cout << ss.str() << std::endl;

  return ss.str(); }

std::string printBmdsStruct(struct python_continuous_model_result *pyRes, bool
		print) { const int colWidth = 8; const int largeColWidth = 14;

  std::stringstream ss;

  ss << std::endl << "Struct: python_continuous_model_result" << std::endl; ss
	  << "model:" << pyRes->model << std::endl; ss << "dist:" <<
	  pyRes->dist << std::endl; ss << "nparms:" << pyRes->nparms <<
	  std::endl; ss << "parms" << std::endl; for (int i = 0; i <
			  pyRes->parms.size(); i++) { printElement(ss, "i:",
			  colWidth); printElement(ss, pyRes->parms[i],
				  colWidth); ss << std::endl; } ss << "cov" <<
				  std::endl; int matSize =
				  sqrt(pyRes->cov.size()); for (int i = 0; i <
						  matSize; i++) { for (int j =
							  0; j < matSize; j++)
						  { printElement(ss,
								  pyRes->cov[i
  ...
  "max:" << pyRes->max << std::endl; ss << "dist_numE:" << pyRes->dist_numE <<
	  std::endl; ss << "model_df:" << pyRes->model_df << std::endl; ss <<
	  "total_df:" << pyRes->total_df << std::endl; ss << "bmd:" <<
	  pyRes->bmd << std::endl; ss << printBmdsStruct(&pyRes->bmdsRes,
			  false); if (pyRes->gof.dose.size() > 0) { ss <<
		  printBmdsStruct(&pyRes->gof, false); } if
		  (pyRes->aod.LL.size() > 0) { ss <<
			  printBmdsStruct(&pyRes->aod, false); }

  ss << std::endl << "bmd_dist" << std::endl; printElement(ss, "Percentile",
		  largeColWidth); printElement(ss, "Value", largeColWidth); ss
	  << std::endl; for (int i = 0; i < pyRes->dist_numE; i++) {
		  printElement(ss, pyRes->bmd_dist[i + pyRes->dist_numE],
				  largeColWidth); printElement(ss,
					  pyRes->bmd_dist[i], largeColWidth);
		  ss << std::endl; }
...
std::string printBmdsStruct(struct python_dichotomous_analysis *pyAnal, bool
		print) { const int colWidth = 8; std::stringstream ss;

  ss << std::endl << "Struct: python_dichotomous_analysis" << std::endl; ss <<
	  "model:" << pyAnal->model << std::endl; ss << "n:" << pyAnal->n <<
	  std::endl; ss << "Data:" << std::endl; printElement(ss, "doses",
			  colWidth); printElement(ss, "Y", colWidth);
  printElement(ss, "n_group", colWidth); ss << std::endl; for (int i = 0; i <
		  pyAnal->doses.size(); i++) { printElement(ss,
		  pyAnal->doses[i], colWidth); printElement(ss, pyAnal->Y[i],
			  colWidth); printElement(ss, pyAnal->n_group[i],
				  colWidth); ss << std::endl; }

  ss << "prior:" << std::endl; if (pyAnal->prior.size() > 0) { for (int i = 0;
		  i < pyAnal->prior.size() / pyAnal->prior_cols; i++) { for
	  (int j = 0; j < pyAnal->prior_cols; j++) { printElement(ss,
			  pyAnal->prior[i * pyAnal->prior_cols + j], colWidth);
	  } ss << std::endl; } } else { ss << "   prior size is zero" <<
		  std::endl; } ss << "BMR:" << pyAnal->BMR << std::endl; ss <<
			  "alpha:" << pyAnal->alpha << std::endl; ss <<
			  "samples:" << pyAnal->samples << std::endl; ss <<
			  "degree:" << pyAnal->degree << std::endl; ss <<
			  "burnin:" << pyAnal->burnin << std::endl; ss <<
			  "parms:" << pyAnal->parms << std::endl; ss <<
			  "prior_cols:" << pyAnal->prior_cols << std::endl; ss
			  << "countAllParmsOnBoundary:" <<
			  (pyAnal->countAllParmsOnBoundary ? "true" : "false")
			  << std::endl;
...
std::string printBmdsStruct(struct python_dichotomous_model_result *pyRes, bool
		print) { const int colWidth = 8; const int largeColWidth = 14;
	std::stringstream ss;

  ss << std::endl << "Struct: python_dichotomous_model_result" << std::endl; ss
	  << "model:" << pyRes->model << std::endl; ss << "nparms:" <<
	  pyRes->nparms << std::endl; ss << "parms" << std::endl; for (int i =
			  0; i < pyRes->parms.size(); i++) { printElement(ss,
			  "i:", colWidth); printElement(ss, pyRes->parms[i],
				  colWidth); ss << std::endl; } ss << "cov" <<
				  std::endl; int matSize =
				  sqrt(pyRes->cov.size()); for (int i = 0; i <
						  matSize; i++) { for (int j =
  ...
  "max:" << pyRes->max << std::endl; ss << "dist_numE:" << pyRes->dist_numE <<
	  std::endl; ss << "model_df:" << pyRes->model_df << std::endl; ss <<
	  "total_df:" << pyRes->total_df << std::endl; ss << "bmd:" <<
	  pyRes->bmd << std::endl; ss << "gof_p_value:" << pyRes->gof_p_value
	  << std::endl; ss << "gof_chi_sqr_statistic:" <<
	  pyRes->gof_chi_sqr_statistic << std::endl; ss <<
	  printBmdsStruct(&pyRes->bmdsRes, false); ss <<
	  printBmdsStruct(&pyRes->gof, false); ss <<
	  printBmdsStruct(&pyRes->aod, false);
...
std::string printBmdsStruct(struct python_dichotomousMA_analysis *pyMA, bool
		print) { const int colWidth = 13; std::stringstream ss;

  ss << std::endl << "Struct: python_dichotomousMA_analysis" << std::endl; ss
	  << "nmodels:" << pyMA->nmodels << std::endl;

  bool printNparms = false; if (pyMA->nparms.size() > 0) { printNparms = true;
  }

  printElement(ss, "models", colWidth); if (printNparms) { printElement(ss,
		  "nparms", colWidth); } printElement(ss, "actual_parms",
			  colWidth); printElement(ss, "prior_cols", colWidth);
  printElement(ss, "modelPriors", colWidth); ss << std::endl;

  for (int i = 0; i < pyMA->nmodels; i++) { printElement(ss, pyMA->models[i],
		  colWidth); if (pyMA->nparms.size() > i) { printElement(ss,
			  pyMA->nparms[i], colWidth); } printElement(ss,
				  pyMA->actual_parms[i], colWidth);
  printElement(ss, pyMA->prior_cols[i], colWidth); printElement(ss,
		  pyMA->modelPriors[i], colWidth); ss << std::endl; }

  ss << "priors:" << std::endl; for (int k = 0; k < pyMA->nmodels; k++) { ss <<
	  "model:" << k << std::endl; for (int i = 0; i <
			  pyMA->priors[k].size() / pyMA->prior_cols[k]; i++) {
		  for (int j = 0; j < pyMA->prior_cols[k]; j++) {
			  printElement(ss, pyMA->priors[k][i *
					  pyMA->prior_cols[k] + j], colWidth);
		  } ss << std::endl; } }
...
std::string printBmdsStruct(struct python_dichotomousMA_result *pyRes, bool
		print) { const int colWidth = 10; const int largeColWidth = 14;
	std::stringstream ss;

  ss << std::endl << "Struct: python_dichotomousMA_result" << std::endl;

  ss << "nmodels:" << pyRes->nmodels << std::endl; ss << "dist_numE:" <<
	  pyRes->dist_numE << std::endl;

  for (int i = 0; i < pyRes->post_probs.size(); i++) { ss << "model:" << i <<
	  ", post_probs:" << pyRes->post_probs[i] << std::endl; }
  ...
  // ind model res
  ss << std::endl << "models" << std::endl; for (int i = 0; i < pyRes->nmodels;
		  i++) { ss << "model:" << i << std::endl; ss <<
...
std::string printBmdsStruct(struct python_continuousMA_analysis *pyMA, bool
		print) { const int colWidth = 13; std::stringstream ss;

  ss << std::endl << "Struct: python_continuousMA_analysis" << std::endl; ss <<
	  "nmodels:" << pyMA->nmodels << std::endl;
  ...
  printElement(ss, "loud_dist_type", colWidth); ss << std::endl;

  for (int i = 0; i < pyMA->nmodels; i++) { printElement(ss, pyMA->models[i],
		  colWidth); printElement(ss, pyMA->loud_dist_type[i],
			  colWidth);
  ...
  ss << "weightOption:" << pyMA->weightOption << std::endl; ss << "datatype:"
	  << pyMA->datatype << std::endl;
...
std::string printBmdsStruct(struct python_continuousMA_result *pyRes, bool
		print) { const int colWidth = 10; const int largeColWidth = 14;
	std::stringstream ss;
  ...
  // bmd_dist
  ss << std::endl << "post_probs" << std::endl; for (int i = 0; i <
		  pyRes->post_probs.size(); i++) { printElement(ss,
		  pyRes->post_probs[i], largeColWidth); ss << std::endl; } ss
		  << std::endl << "bmd_dist" << std::endl; for (int i = 0; i <
				  pyRes->bmd_dist.size(); i++) {
...
std::string printBmdsStruct(struct python_multitumor_analysis *pyAnal, bool
		print) { const int colWidth = 10; std::stringstream ss;

  ss << std::endl << "Struct: python_multitumor_analysis" << std::endl;

  ss << "ndatasets:" << pyAnal->ndatasets << std::endl;

  printElement(ss, "model", colWidth); printElement(ss, "n", colWidth);
  printElement(ss, "nmodels", colWidth); printElement(ss, "degree", colWidth);
  ss << std::endl; for (int i = 0; i < pyAnal->ndatasets; i++) {
	  printElement(ss, i, colWidth); printElement(ss, pyAnal->n[i],
			  colWidth); printElement(ss, pyAnal->nmodels[i],
				  colWidth); printElement(ss,
					  pyAnal->degree[i], colWidth); ss <<
					  std::endl; }

  ss << "BMD_type:" << pyAnal->BMD_type << std::endl; ss << "BMR:" <<
	  pyAnal->BMR << std::endl; ss << "alpha:" << pyAnal->alpha <<
	  std::endl; ss << "prior_cols:" << pyAnal->prior_cols << std::endl;
...
{ const int colWidth = 10; std::stringstream ss;

  ss << std::endl << "Struct: python_multitumor_result" << std::endl;

  ss << "ndatasets:" << pyRes->ndatasets << std::endl;

  printElement(ss, "model", colWidth); printElement(ss, "nmodels", colWidth);
  printElement(ss, "selectedModelIndex", colWidth); ss << std::endl; for (int i
		  = 0; i < pyRes->ndatasets; i++) { printElement(ss, i,
			  colWidth); printElement(ss, pyRes->nmodels[i],
				  colWidth); printElement(ss,
					  pyRes->selectedModelIndex[i],
					  colWidth); ss << std::endl; }

  ss << "BMD:" << pyRes->BMD << std::endl; ss << "BMDL:" << pyRes->BMDL <<
	  std::endl; ss << "BMDU:" << pyRes->BMDU << std::endl; ss <<
	  "slopeFactor:" << pyRes->slopeFactor << std::endl; ss <<
	  "combined_LL:" << pyRes->combined_LL << std::endl; ss <<
	  "combined_LL_const:" << pyRes->combined_LL_const << std::endl;

  // ind model res
  ss << std::endl << "models" << std::endl; for (int j = 0; j <
		  pyRes->ndatasets; j++) { ss << "dataset:" << j << std::endl;
	  for (int i = 0; i < pyRes->nmodels[j]; i++) { ss << "model:" << i <<
		  std::endl; ss << printBmdsStruct(&pyRes->models[j][i],
				  false); } }
...
{ const int colWidth = 10;
  ...
  ss << "Data:" << std::endl; printElement(ss, "doses", colWidth);
  printElement(ss, "incidence", colWidth); printElement(ss, "litterSize",
		  colWidth); printElement(ss, "lsc", colWidth); ss <<
	  std::endl; for (int i = 0; i < pyAnal->doses.size(); i++) {
		  printElement(ss, pyAnal->doses[i], colWidth);
		  printElement(ss, pyAnal->incidence[i], colWidth);
		  printElement(ss, pyAnal->litterSize[i], colWidth);
		  printElement(ss, pyAnal->lsc[i], colWidth); ss << std::endl;
	  }

  ss << "LSC_type:" << pyAnal->LSC_type << std::endl; ss << "ILC_type:" <<
	  pyAnal->ILC_type << std::endl; ss << "BMD_type:" << pyAnal->BMD_type
	  << std::endl; ss << "estBackground:" << (pyAnal->estBackground ?
			  "True" : "False") << std::endl; ss << "parms:" <<
	  pyAnal->parms << std::endl; ss << "prior_cols:" << pyAnal->prior_cols
	  << std::endl; ss << "BMR:" << pyAnal->BMR << std::endl; ss <<
	  "alpha:" << pyAnal->alpha << std::endl; ss << "numBootRuns:" <<
	  pyAnal->numBootRuns << std::endl; ss << "iterations:" <<
	  pyAnal->iterations << std::endl; ss << "seed:" << pyAnal->seed <<
	  std::endl; ss << "countAllParmsOnBoundary:" <<
	  (pyAnal->countAllParmsOnBoundary ? "True" : "False") << std::endl;

  ss << "prior:" << std::endl; for (int i = 0; i < pyAnal->prior.size() /
		  pyAnal->prior_cols; i++) { for (int j = 0; j <
			  pyAnal->prior_cols; j++) { printElement(ss,
				  pyAnal->prior[i * pyAnal->prior_cols + j],
				  colWidth); } ss << std::endl; }
...
std::string printBmdsStruct(struct python_nested_result *pyRes, bool print) {
	const int colWidth = 10; const int largeColWidth = 14;

  std::stringstream ss; ss << std::endl << "Struct: python_nested_result" <<
	  std::endl;

  ss << "validResult:" << (pyRes->validResult ? "True" : "False") << std::endl;
  ss << "model:" << pyRes->model << std::endl; ss << "nparms:" << pyRes->nparms
	  << std::endl; ss << "parms" << std::endl; printElement(ss, "parm",
			  colWidth); printElement(ss, "value", colWidth); ss <<
	  std::endl; for (int i = 0; i < pyRes->parms.size(); i++) {
		  printElement(ss, i, colWidth); printElement(ss,
				  pyRes->parms[i], colWidth); ss << std::endl;
	  }

  ss << "cov" << std::endl; int matSize = sqrt(pyRes->cov.size()); for (int i =
		  0; i < matSize; i++) { for (int j = 0; j < matSize; j++) {
	  printElement(ss, pyRes->cov[i * matSize + j], largeColWidth); } ss <<
		  std::endl; }

  ss << "dist_numE:" << pyRes->dist_numE << std::endl; ss << "model_df:" <<
	  pyRes->model_df << std::endl; ss << "bmd:" << pyRes->bmd <<
	  std::endl; ss << "fixedLSC:" << pyRes->fixedLSC << std::endl; ss <<
	  "LL:" << pyRes->LL << std::endl; ss << "combPVal:" << pyRes->combPVal
	  << std::endl;

  // structs
  ss << printBmdsStruct(&pyRes->bmdsRes, false); ss <<
	  printBmdsStruct(&pyRes->litter, false); ss <<
	  printBmdsStruct(&pyRes->boot, false); ss <<
	  printBmdsStruct(&pyRes->reduced, false); ss <<
	  printBmdsStruct(&pyRes->srData, false);
