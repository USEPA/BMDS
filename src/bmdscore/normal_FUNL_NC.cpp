...
*           can be performed. This function is used in an optimization that finds the
                  ...
                  that satisfies the equality constraint. Note it always modifies parameters that
are essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_absolute(
...
std::vector<double> normalFUNL_BMD_NC::bmd_start_absolute_clean(
...
*           can be performed. This function is used in an optimization that finds the
            closest point to the supplied value (usually the previous MLE)
            that satisfies the equality constraint. Note it always modifies parameters that are
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_reldev(unsigned n, const double *b, double *grad, void *data) {
  start_data *sdata = (start_data *)data;
  Eigen::MatrixXd theta = sdata->theta;
  ...
  returnV += pow(theta(6, 0) - b[6], 2);  // variance parameter
  temp =
      exp(-b6 * (sdata->BMD - b5) * (sdata->BMD - b5)) * (1 / (1 + exp(-(sdata->BMD - b3) / b4)));
...
std::vector<double> normalFUNL_BMD_NC::bmd_start_reldev_clean(
    std::vector<double> b, double BMRF, double BMD, bool isIncreasing
) {
  double t;
  ...
  temp -= (1 + t) * exp(-exp(b[5]) * (0.0 - b[4]) * (0.0 - b[4])) *
          (1 / (1 + exp(-(0.0 - b[2]) / b[3])));
  temp = b[0] * t / temp;
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_stddev(unsigned n, const double *b, double *grad, void *data) {
  start_data *sdata = (start_data *)data;
...
std::vector<double> normalFUNL_BMD_NC::bmd_start_stddev_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing
) {
  if (!isIncreasing) BMRF *= 1;
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_point(unsigned n, const double *b, double *grad, void *data) {
  start_data *sdata = (start_data *)data;
  Eigen::MatrixXd theta = sdata->theta;
  ...
  returnV += pow(b6 - b[5], 2);
  returnV += pow(theta(6, 0) - b[6], 2);  // variance parameter
  temp = (sdata->BMRF) / exp(-exp(b[5]) * (sdata->BMD - b[4]) * (sdata->BMD - b[4])) *
         (1 / (1 + exp(-(sdata->BMD - b[2]) / b[3])));
...
std::vector<double> normalFUNL_BMD_NC::bmd_start_point_clean(
    std::vector<double> b, double BMRF, double BMD, bool isIncreasing
) {
  double temp = 0;
  temp = (BMRF) / exp(-exp(b[5]) * (BMD - b[4]) * (BMD - b[4])) *
         (1 / (1 + exp(-(BMD - b[2]) / b[3])));
  b[0] = temp;
  return b;
}
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_extra(unsigned n, const double *b, double *grad, void *data) {
  start_data *sdata = (start_data *)data;
  Eigen::MatrixXd theta = sdata->theta;
...
std::vector<double> normalFUNL_BMD_NC::bmd_start_extra_clean(
    std::vector<double> x, double BMRF, double BMD, bool isIncreasing
) {
  double temp = 0.0;
...
essentially unbounded - nu
*/
double normalFUNL_BMD_NC::bmd_start_hybrid_extra(
    unsigned n, const double *b, double *grad, void *data
) {
  start_data *sdata = (start_data *)data;
...
std::vector<double> normalFUNL_BMD_NC::bmd_start_hybrid_extra_clean(
...
// code
//            in  normalFUNL_BMD_NC::bmd_absolute(Eigen::MatrixXd theta, double BMRF, bool
//            isIncreasing)
//
double normalFUNL_BMD_NC::bmd_absolute_bound(
...
double normalFUNL_BMD_NC::bmd_stdev_bound(
    Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing
) {
...
double normalFUNL_BMD_NC::bmd_reldev_bound(
    Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing
) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalFUNL_BMD_NC::bmd_extra_bound(
    Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing
) {
  Eigen::MatrixXd d(1, 1);
  d(0, 0) = 0.0;
...
double normalFUNL_BMD_NC::bmd_point_bound(
    Eigen::MatrixXd theta, double BMD, double BMRF, bool isIncreasing
) {
...
//
////////////////////////////////////////////////////////////////////////
double normalFUNL_BMD_NC::bmd_hybrid_extra_bound(
...
//
////////////////////////////////////////////////////////////////////////
double normalFUNL_BMD_NC::bmd_hybrid_extra(
  ...
  // Get the mean and variance at dose zero as well as a very high dose
  double min_d = 0.0;
  double max_d = findOptim(theta, isIncreasing);
  ;
  ...
  double temp_const =
      (isIncreasing) ? inv_bprob * std_zero + mu_zero : mu_zero - inv_bprob * std_zero;
