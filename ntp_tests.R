...
dd = shirley_ntp(numeric_value ~ sex + phase_time+ endpoint, data=jonck_input.shirley_dunn)
#BB = dunnett_ntp(numeric_value ~ sex + endpoint,data=jonck_input.shirley_dunn)
CC = williams_ntp(numeric_value ~ sex + phase_time + endpoint,data=jonck_input.shirley_dunn)
