...
f1 = function(x){log(exp(x)*expm1(x))-(log(37.5^2) -2*log(481))}
uniroot(f1,c(0,1))
f2 = function(x){log(exp(x)*expm1(x))-(log(1.1454^2) -2*log(10.58))}
uniroot(f2,c(0,1))
...
\end{align}
For the body-weight conditions $\sigma = 37.5,$ and for the liver weight conditions,
$\sigma = 1.145.$
...
\end{align}
For the body weight conditions $\sigma = 0.0777$, and for the liver weight conditions
$\sigma = 0.107302.$ These values were chosen so that the control group standard deviation
...
\end{align}
For the body-weight conditions $\lambda = 78643.17,$ and for the liver weight conditions,
$\lambda = 902.3632.$ These values were chosen so that the control group standard deviation
matched with the other simulation conditions, which was the average SD from males and female rats in table 1 at 72 weeks from [Piao et al(2013)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3620211/).
...
y <- rinvgauss(length(mean),mean,78643.17/4)
...
y <- rinvgauss(length(mean),mean,902.3632/4)
...
Exp5.p <- rbind(c(481,0.05,(1/1.42870),2),
              ...
              c(481,0.1,(1/1.42870),2) ,
                c(10.58,0.05,(1.5),1.5),
                c(10.58,0.02,(1.5),1.5),
                c(10.58,0.01,(1.5),1.5),
                c(10.58,0.1, (1.5),1.5))
...
y <- rinvgauss(length(mean),mean,78643.17)
...
y <- rinvgauss(length(mean),mean, 902.3632)
...
kable(BMD_IS,digits=2,caption ="I-Spline Simulation BMDs for standard deviation BMD across
                                the three different distributional conditions.")
```
### Table of BMD values for RD approach
...
kable(BMD_IS,digits=2,caption ="I-Spline Simulation BMDs for standard deviation BMD across
                                the three different distributional conditions.")
```
### Table of BMD values for HB approach
...
for(i in 1:8){
 ...
 if (isM(0) < isM(100)){
     ...
     bb  <-  function(d){((1-pinvgauss(temp,isM(d),lambda[i])) -0.05)/0.95-0.1}
  }else{
      temp <- qinvgauss(0.05,isM(0),lambda[i])
      bb  <-  function(d){((pinvgauss(temp,isM(d),lambda[i]))- 0.05)/0.95-0.1}
  }
  bmd_sd_ig[i] <- uniroot(bb,c(0,100))$root
}
...
for(i in 1:8){
  ...
  if (isM(0) < isM(100)){
     ...
     bb  <-  function(d){((1-pnorm(temp,isM(d),sqrt(var_no[i]))) -0.05)/0.95-0.1}
  }else{
      temp <- qnorm(0.05,isM(0),sqrt(var_no[i]))
      bb  <-  function(d){(pnorm(temp,isM(d),sqrt(var_no[i]))- 0.05)/0.95-0.1}
  }
...
for(i in 1:8){
  ...
  if (isM(0) < isM(100)){
     ...
     bb  <-  function(d){((1-plnorm(temp,log(isM(d)),sqrt(var_ln[i]))) -0.05)/0.95-0.1}
  }else{
      temp <- qlnorm(0.05,log(isM(0)),sqrt(var_ln[i]))
      bb  <-  function(d){(plnorm(temp,log(isM(d)),sqrt(var_ln[i]))- 0.05)/0.95-0.1}
  }
  bmd_sd_ln[i] <- uniroot(bb,c(0,100))$root
}
