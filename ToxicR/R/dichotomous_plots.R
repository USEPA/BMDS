...
plot.BMDdich_fit_MCMC<-function(fit,qprob=0.05,...){
  ...
  if (fit$model=="hill"){
    Q <- apply(fit$PARMS,1,dich_hill_f, d=test_doses)
  }
  if (fit$model=="gamma"){
    Q <- apply(fit$PARMS,1,dich_gamma_f, d=test_doses)
  }
  if (fit$model=="logistic"){
    Q <- apply(fit$PARMS,1,dich_logist_f, d=test_doses)
  }
  if (fit$model=="log-logistic"){
    Q <- apply(fit$PARMS,1,dich_llogist_f, d=test_doses)
  }
  if (fit$model=="probit"){
    Q <- apply(fit$PARMS,1,dich_probit_f, d=test_doses)
  }
  if (fit$model=="log-probit"){
    Q <- apply(fit$PARMS,1,dich_lprobit_f, d=test_doses)
  }
  if (fit$model=="multistage"){
    Q <- apply(fit$PARMS,1,dich_multistage_f, d=test_doses)
  }
  if (fit$model=="qlinear"){
    Q <- apply(fit$PARMS,1,dich_qlinear_f, d=test_doses)
  }
  if (fit$model=="weibull"){
    Q <- apply(fit$PARMS,1,dich_weibull_f, d=test_doses)
  }
  ...
  if (BMD_DENSITY ==TRUE){
    Dens =  density(fit$BMDS,cut=c(max(doses)))
...
plot.BMDdich_fit_mle<-function(fit,qprob=0.05,...){
...
plot.BMDdich_fit_laplace<-function(fit,qprob=0.05,...){
